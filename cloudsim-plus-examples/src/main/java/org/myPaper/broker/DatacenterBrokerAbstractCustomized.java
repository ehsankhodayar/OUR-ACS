 ...
 */
public abstract class DatacenterBrokerAbstractCustomized extends CloudSimEntity implements DatacenterBroker {
    protected boolean selectClosestDatacenter;
     ...
     */
    protected Datacenter lastSelectedDc;
    ...
    /**
     * @see #getVmWaitingAckList()
     */
    private final List<Vm> vmWaitingAckList;

    /** @see #getVmWaitingList() */
    protected final List<Vm> vmWaitingList;
    ...
    protected BiFunction<Datacenter, Vm, Datacenter> datacenterMapper;
     ...
     */
    public DatacenterBrokerAbstractCustomized(final CloudSim simulation, final String name) {
        super(simulation);
        if(!name.isEmpty()) {
            setName(name);
        }
        ...
        this.vmWaitingAckList = new ArrayList<>();
    ...
    private boolean processVmCreateResponseFromDatacenter(final SimEvent evt) {
        ...
        if (vm.isCreated()) {
            ...
            if(!retryFailedVms){
                ...
                List<Cloudlet> relatedCloudlets = getCloudletWaitingList().stream()
                    .filter(cloudlet -> cloudlet.getVm() == vm)
                    .collect(Collectors.toList());
                cloudletWaitingList.removeAll(relatedCloudlets);

                LOGGER.warn("{}: Cloudlet Failure: List of {} bounded cloudlet(s) to {} failed due to the lack of suitable host!",
                    getSimulation().clockStr(), relatedCloudlets.size(), vm);
            }
     ...
     */
    int requestVmCreation(final Datacenter datacenter, final boolean isFallbackDatacenter, final Vm vm) {
    ...
    /**
     * Gets the list of VMs waiting for receiving their acks.
     *
     * @return the list of waiting ack VMs
     */
    protected List<Vm> getVmWaitingAckList() {
        return vmWaitingAckList;
    }
